...
function navigate($listing_id, $itemid, $direction, $index_name, $url)
{
    ...
    if (!in_array($direction, array('forward','backward'))) return false;

    // Get the keys
    $data = xarSession::getVar('listing.lastkeys');
    if (!isset($data[$listing_id])) return false;
  
    $current_place = array_search($itemid, $data[$listing_id]['keys']);
    $next_key = false;
    if ($current_place !== false) {
        if ($direction == 'forward') {
            if ($current_place < count($data[$listing_id]['keys']) - 1) $next_place = $current_place + 1;
            else $next_place = false;
        } elseif ($direction == 'backward') {
            if ($current_place > 0) $next_place = $current_place - 1;
            else $next_place = false;
        }
        if ($next_place !== false) $next_key = $data[$listing_id]['keys'][$next_place];
    }
    if ($next_key == false) return false;
    
    if (empty($url)) {
        $url = xarServer::getCurrentURL(array($index_name => $next_key));
    } else {
        ...
        $querystring = '';
        foreach ($queryparts as $key => $value) $querystring .= "&" . $key . "=" . $value;
