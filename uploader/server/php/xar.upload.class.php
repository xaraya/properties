...
class xarUploadHandler
{
    ...
    protected function set_file_delete_url($file) {
        ...
        $file->delete_url = $this->options['script_url']
            .'?file='.rawurlencode($file->name).'&dir='.rawurlencode($this->options['upload_dir']).'&thumbdir='.rawurlencode($this->options['image_versions']['thumbnail']['upload_dir']);
    ...
    public function delete() {
        $file_name = isset($_REQUEST['file']) ?
            basename(stripslashes($_REQUEST['file'])) : null;
        $file_path = isset($_REQUEST['dir']) ?
            stripslashes($_REQUEST['dir']) : null;
        $file_path .= $file_name;
        $success = is_file($file_path) && $file_name[0] !== '.' && unlink($file_path);
        if ($success) {
            $file = isset($_REQUEST['thumbdir']) ?
                stripslashes($_REQUEST['thumbdir']) : null;
            $file .= $file_name;
