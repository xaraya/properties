...
class Securimage {
    ...
    // XARAYA adaption: add this relative path and modfiy the paths to the ttf and gd fonts
    // Word list path is not touched
    const xarrelpath = 'modules/secureinput/class/securimage/';
  ...
  function drawWord()
  {
    if ($this->use_gd_font == true) {
      if (!is_int(self::xarrelpath . $this->gd_font_file)) { //is a file name
        $font = @imageloadfont(self::xarrelpath . $this->gd_font_file);
        if ($font == false) {
          trigger_error("Failed to load GD Font file {$this->gd_font_file} ", E_USER_WARNING);
          return;
        }
      } else { //gd font identifier
        $font = self::xarrelpath . $this->gd_font_file;
      }
      ...
      for($i = 0; $i < $strlen; ++$i) {
        ...
        @imagettftext($this->im, $this->font_size, $angle, $x, $y, $font_color, self::xarrelpath . $this->ttf_file, $this->code{$i});
